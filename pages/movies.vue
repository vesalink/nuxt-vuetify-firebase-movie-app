<template>
  <v-layout>
    <v-container v-if="$store.state.loading" class="text-center">
    ...
    <v-container v-else>
      <v-alert v-if="$store.state.searchText !== ''" type="info">
        Search results for keyword: {{ $store.state.searchText }}
      </v-alert>
      <v-alert
        v-if="$store.state.searchText === 'Joker'"
        border="bottom"
        colored-border
        type="warning"
        elevation="2"
      >
        By default the app searching for 'Joker'... Start typing the movie
        keyword and press Enter in the search box above
      </v-alert>
      <v-dialog
        v-model="showDetails"
        overlay-color="black"
            ...
            <v-row justify="space-between">
              ...
              <v-col xl="8" lg="8" md="8" sm="12" xs="12">
                <div class="justify-space-between">
                  ...
                  <v-card-actions>
                    <v-btn @click="showDetails = false" color="orange">
                      Close
                    </v-btn>
                  </v-card-actions>
                </div>
        ...
        >
          <v-card class="pa-6" dark raised>
            <div>
              ...
              <v-card-actions>
                <v-btn
                  @click.stop="searchingMovies(movie.imdbID)"
                  color="orange"
                >
                  Details
                </v-btn>
